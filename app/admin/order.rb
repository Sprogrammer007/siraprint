        ...
        if order.ordered_products.large_format  
          ...
          table_for order.ordered_products.large_format do
            ...
            column "User Design" do |p|
              if !p.print_pdf.nil?
                link_to File.basename(p.print_pdf), p.print_pdf.splice('/').pop().push(CGI.escape(File.basename(p.print_pdf))).join("/")
              else
                "Ask to Upload"
              end
            end
            column "User Design 2" do |p|
              if !p.print_pdf_2.nil?
                link_to File.basename(p.print_pdf_2), p.print_pdf_2[-1].replace(CGI.escape(File.basename(p.print_pdf_2)))
              else
                "None"
              end
            end
